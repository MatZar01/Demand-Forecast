from base_src import MLP_dataset_emb_2
from base_src import get_matches
from base_src import MLP_emb, MLP_emb_tl_2
...
for m in matches:
    try:
        train_data = MLP_dataset_emb_2(path=DATA_PATH, train=True, lag=LAG, get_quant=QUANT, normalize=NORMALIZE,
                                     embedders=embedders, matches=m)
        val_data = MLP_dataset_emb_2(path=DATA_PATH, train=False, lag=LAG, get_quant=QUANT, normalize=NORMALIZE,
                                   embedders=embedders, matches=m)
        ...
        model = MLP_emb_tl_2(input_dim=train_data.sample_item, cat_2_size=train_data.cat_2_size, cat_3_size=train_data.cat_3_size, embedding_size=5)
        ...
        if m is None:
            out_dict[f'whole_model'] = light_model.out_dict
        else:
