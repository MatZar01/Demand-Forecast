...
def get_store_sku_match(data_manager):
    ...
    pairs = data_train[:, [1, 2]].astype(int)
...
def get_data(data_manager, ids: list):
    ...
    if train_single.shape[0] > 1500:
        train_single = train_single[-1500:, :]
    if val_single.shape[0] > 1000:
        val_single = val_single[:1000, :]

    # get only relevant data
    train_single_X, train_single_y = train_single[:, [1, 2]], train_single[:, [-1]]
    val_single_X, val_single_y = val_single[:, [1, 2]], val_single[:, [-1]]
...
if __name__ == '__main__':
    ...
    CFG_FILE = '../cfgs/default_3.yml'
    SAVE_DIR = './results'
    task_info = get_args(CFG_FILE)
    Data_manager = DataSet_3(paths=task_info['DATA_PATH'], year_split=True, encode=True)
    result_dict = {}
    ...
    for match in c_prod:
        ...
        try:
            ...
            tree_pruned, out_pruned = None, None
