...
parser.add_argument('-R', '--results_per_seed', action='store_false', help="Read results per seed")
...
if args.results_per_seed:
    for idx, incremental_learners in enumerate(results['incremental_learners_per_seed']):
        for l_name, l in incremental_learners.items():
            ...
            learners[f'F_inc({l_name}):{idx}'] = ll
else:
...
cumulative_df = plot_windowed_results(*final_results_p,
                      metric="rmse",
                      ylabel='RMSE',
                      plot_aggregated=True if args.results_per_seed else False,
